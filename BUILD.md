...
```bash
# 构建所有（推荐）- 会自动将mac-file-search打包到APP内
make

# 或者分别构建
make scanner  # 只构建命令行工具
make app      # 构建GUI应用（会自动打包mac-file-search）
```
...
1. **命令行扫描工具**（两个位置）：
   - `./mac-file-search` - 当前目录
   - `./mac-search-app/bin/mac-file-search` - 开发环境副本
...
**APP已包含**：
- ✓ 主程序
- ✓ mac-file-search扫描工具（自动打包在Resources目录）
- ✓ 所有依赖
...
cp mac-file-search mac-search-app/bin/
```
...
```bash
# 扫描/Applications目录
sudo ./mac-file-search -path /Applications -output result.json

# 扫描根目录，排除外置硬盘
sudo ./mac-file-search -path / -output result.json -exclude /Volumes/MacExtDisk

# 显示错误详情
sudo ./mac-file-search -path / -output result.json -errors
```
...
| 新方案（内嵌mac-file-search） | **2-3 分钟** | 1次 |
| **性能提升** | **5倍** | **1000倍减少** |

### 技术细节

- **无需外部依赖**：mac-file-search已嵌入APP包
...
```bash
# 检查mac-file-search是否在APP包内
ls -lh mac-search-app/build/bin/mac-search-app.app/Contents/Resources/mac-file-search

# 应该显示：
# -rwxr-xr-x  1 user  staff   2.9M  日期 时间 mac-file-search
```
...
**解决**：
- GUI应用会提示输入sudo密码
- 命令行工具需要用 `sudo ./mac-file-search` 运行
